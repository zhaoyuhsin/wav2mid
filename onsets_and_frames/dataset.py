    ...
    def __init__(self, path, groups=None, sequence_length=None, seed=42, device=DEFAULT_DEVICE):
        ...
        for group in groups:
            ...
            for input_files in tqdm(files, desc='Loading group %s' % group):
                self.fx.append(input_files)
                t=self.load(*input_files)
        print("self.fx len = ", len(self.fx))

    def __getitem__(self, index):
        data = self.load(*self.fx[index])
...
class MAESTRO(PianoRollAudioDataset):
    ...
    def files(self, group):  # return Num * （audio_path, tsv_filename）
...
class MAPS(PianoRollAudioDataset):
    ...
    def files(self, group):
        flacs = glob(os.path.join(self.path, 'flac', '*.flac'))
